...
void calculateOutput(string* outputFilePath, unsigned int numberOfImages, vector<Image *> *imagesOG, vector<Image *> *imagesNew,  vector<Image *> *queryImagesOG, vector<Image *> *queryImagesNew, LSH* myLSH, unsigned int k, unsigned int N){
    unsigned int numberOfQueries, index;
    int offset;
    Image* LSHNearestNeighbor;
    vector<Image*> *kActualNearestNeighbors;
    Image* ActualReducedNearestNeighbor;
    Image* ActualNearestNeighbor;

    //open output file in write mode
    ofstream *outputFile = new ofstream(outputFilePath->c_str(), ios::out);

    double tReduced = 0, tLSH = 0, tTrue = 0;
    unsigned int distApproximateNNNeighbor, distApproximateLSHNeighbor, distTrueNeighbor;
    double sumReduced = 0, sumLSH = 0;
    double approximationFacReduced, approximationFacLSH;

    unsigned int numOfQueryImages = queryImagesOG->size();
    //For every query Image
    for(index = 0; index < numOfQueryImages; index++){
        *outputFile << "Query: " << queryImagesOG->at(index)->id + 1<< endl;

        // Get starting timepoint 
        auto start1 = high_resolution_clock::now();
        
        // get the nearest neighbor in new and reduced space
        kActualNearestNeighbors = exhaustiveNearestNeighbors(queryImagesNew->at(index), imagesNew, 1);
        ActualReducedNearestNeighbor = kActualNearestNeighbors->at(0);
        ...
        // get the LSH nearest neighbor in original space
        LSHNearestNeighbor = myLSH->NearestNeighbor(queryImagesOG->at(index));
        ...
        // get the nearest neighbor in original space
        kActualNearestNeighbors = exhaustiveNearestNeighbors(queryImagesOG->at(index), imagesOG, 1);
        ActualNearestNeighbor = kActualNearestNeighbors->at(0);

        // Get ending timepoint 
        auto stop3 = high_resolution_clock::now();

        //Calculate durations
        auto durationReduced = duration_cast<microseconds>(stop1 - start1);
        auto durationLSH = duration_cast<microseconds>(stop2 - start2);
        auto durationTrue = duration_cast<microseconds>(stop3 - start3);

        tReduced += durationReduced.count();
        tLSH += durationLSH.count();
        tTrue += durationTrue.count();

        // Write results to output
        *outputFile << "Nearest neighbor Reduced: " << ActualReducedNearestNeighbor->id << endl;
        if(LSHNearestNeighbor == NULL){
            *outputFile << "Nearest neighbor LSH: Not Found" << endl;
        } else {
            *outputFile << "Nearest neighbor LSH: " << LSHNearestNeighbor->id << endl;
        }
        *outputFile << "Nearest neighbor True: " << ActualNearestNeighbor->id << endl;

        // Calculate distances
        distApproximateNNNeighbor = DIST_FUNC(queryImagesOG->at(index), imagesOG->at(ActualReducedNearestNeighbor->id));
        
        if(LSHNearestNeighbor == NULL){
            distApproximateLSHNeighbor = 0;
        } else {
            distApproximateLSHNeighbor = DIST_FUNC(queryImagesOG->at(index), LSHNearestNeighbor);
        }
        
        distTrueNeighbor = DIST_FUNC(queryImagesOG->at(index), ActualNearestNeighbor);

        // Write results to output
        *outputFile << "distanceReduced: " << distApproximateNNNeighbor << endl;  
        *outputFile << "distanceLSH: " << distApproximateLSHNeighbor << endl;
        *outputFile << "distanceTrue: " << distTrueNeighbor << endl << endl << endl;

        // Calculate factors
        sumReduced += (double)((double)distApproximateNNNeighbor / (double)distTrueNeighbor);
        sumLSH += (double)((double)distApproximateLSHNeighbor / (double)distTrueNeighbor);
        
    }

    // Write results to output
    *outputFile << endl << "tReduced: " << tReduced << endl;
    *outputFile << "tLSH: " << tLSH << endl;
    *outputFile << "tTrue: " << tTrue << endl;

    //Calculate approximation Factors
    approximationFacReduced = sumReduced / (double)(numOfQueryImages);
    approximationFacLSH = sumLSH / (double)(numOfQueryImages);

    // Write results to output
    *outputFile << "Approximation Factor LSH: " << approximationFacLSH << endl;
    *outputFile << "Approximation Factor Reduced: " << approximationFacReduced << endl;
}
...
void runClusteringAlgorithm(vector<Image *> *images, vector<Image *> *osImages, bool toOriginalSpace, unsigned int numberOfClusters, string *method, ofstream *outputFile, LSH *myLSH, HyperCube *myHyperCube, int MHyperCube, int probesHyperCube, bool complete){
    ...
    *outputFile << "clustering_time: " << durationClustering.count() * 1.0 * pow(10, -6) << endl; 
    ...
    // we must check and translate new space clusters to original space
    if(toOriginalSpace){
        clusters = originalClusters(clusters, osImages);
        centroids = centroidize(clusters);
    }
    ...
    *outputFile << "]" << endl;
    ...
    // now print objective function
    if(toOriginalSpace){
        *outputFile << "Value of Objective Function: " << objectiveFunction(osImages, centroids) << endl;
    } else {
        *outputFile << "Value of Objective Function: " << objectiveFunction(images, centroids) << endl;
    }
...
vector<set <Image *> *> *originalClusters(vector<set <Image *> *> *inputClusters, vector<Image *> *osImages){
    vector<set <Image *> *> *outputClusters = new vector<set <Image *> *>();
    set<Image *> *outputCluster;
    
    for(set<Image *> *inputCluster : *inputClusters){
        outputCluster = new set<Image *>();
        for(Image *currImage : *inputCluster){
            outputCluster->insert(osImages->at(currImage->id));
        };
        outputClusters->push_back(outputCluster);
    };
...
vector<set <Image *> *> *readClustersFromFile(string *nnFilePath, vector<Image *> *osImages){
    vector<set <Image *> *> *clusters = new vector<set <Image *> *>();
    set<Image *> *currCluster;
    int currImageId;
    

    // read the file line by line (cluster by cluster)
    fstream* nnFile = new fstream(nnFilePath->c_str(), ios::in | ios::binary);

    string* currLine = new string();
    while(getline(*nnFile, *currLine)){
        if(*currLine == ""){
            break;
        }

        
        // go to start of actual image id's
        currLine->erase(0, currLine->find(",") + 1);

        // replace all occurences of , to space
        replace(currLine->begin(), currLine->end(), ',', ' ');

        // now we have the id's of the images separated by spaces
        stringstream imageIdStream(*currLine);

        currCluster = new set<Image *>();

        while((imageIdStream >> currImageId)){
            currCluster->insert(osImages->at(currImageId));
        }
...
unsigned long long int objectiveFunction(vector<Image *> *images, vector<Image *> *centroids){
    int minDist, currDist;
    unsigned long long int sum = 0;

    for(Image *currImage : *images){
        minDist = -1;
        for(Image *currCentroid : *centroids){
            if(minDist == -1){
                minDist = DIST_FUNC(currImage, currCentroid);
            } else {
                currDist = DIST_FUNC(currImage, currCentroid);
                if(currDist < minDist){
                    minDist = currDist;
                }
            }
